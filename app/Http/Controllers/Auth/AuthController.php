...
use App\Http\Requests\AccountForgotPasswordSet;
...
class AuthController extends Controller
{
    ...
    public function Login(AccountLogin $request)
    {
        ...
        try {
            $user = \Sentinel::authenticate($credentials, $request->input('remember_me'));
            \Sentinel::getUserRepository()->recordLogin($user);
            if (\Sentinel::check()) {
                return redirect('/');
            }
        } catch (\Exception $e) {
            \Sentinel::getUserRepository()->findByCredentials($credentials);
    ...
    public function Register(AccountRegister $request)
    {
        try {
            ...
            return redirect('auth/register')->withErrors(array('register' => 'A user with that e-mail already exist!'));
        }
    }
     ...
     *
     * @param AccountForgotPasswordSet $request
     *
     * @return $this
     */
    public function SetForgotPassword(AccountForgotPasswordSet $request)
    {
        try {
            $user = \Sentinel::getUserRepository()->findById($request->input('UserId'));
            if (\Reminder::complete($user, $request->input('ResetCode'), $request->input('password'))) {
